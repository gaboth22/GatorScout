...
} Pwm_ConfigurableTA0CCR2_t;

static Pwm_ConfigurableTA0CCR2_t instance;
...
static void SetDutyCycle(I_Pwm_t *pwm, uint8_t duty)
{
    ...
    TIMER_A0->CCR[2] = 480 * (uint16_t)duty;
}
...
static void ChangePortMap(I_Pwm_t *pwm, GpioChannel_t channel)
{
    ...
    P2->SEL0 |= instance.currentPinMask;
    DoPortMap(instance.currentPinNumber, PMAP_TA0CCR2A);
}

static const PwmApi_t api =
    { SetDutyCycle, ChangePortMap };

I_Pwm_t * Pwm_ConfigurableTA0CCR2_Init(GpioChannel_t channel)
{
    ...
    DoPortMap(pinInfo.pinNumber, PMAP_TA0CCR2A);

    TIMER_A0->CCTL[2] = TIMER_A_CCTLN_OUTMOD_6; // CCR2 toggle/set
    TIMER_A0->CCR[2] = 0; // Duty cycle of zero
