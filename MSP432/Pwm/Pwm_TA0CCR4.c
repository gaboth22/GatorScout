...
static void SetDutyCycle(I_Pwm_t *pwm, uint8_t duty)
{
    ...
    TIMER_A0->CCR[4] = 480 * (uint16_t)duty;
}
...
I_Pwm_t * Pwm_TA0CCR4_Init(GpioChannel_t channel)
{
    ...
    DoPortMapOnPort2(pinInfo.pinNumber, PMAP_TA0CCR4A);

    TIMER_A0->CCTL[4] = TIMER_A_CCTLN_OUTMOD_6; // CCR2 toggle/set
    TIMER_A0->CCR[4] = 0; // Duty cycle of zero
